...
async function runWsTest() {
	...
	try {
		...
		setStatus(
			"ws",
			"running",
			`echo ok (${rtt1.toFixed(0)}ms) — waiting 30s to test reconnect`,
		);
		...
		setStatus(
			"ws",
			"pass",
			`connect ${rtt1.toFixed(0)}ms, reconnect+echo ${rtt2.toFixed(0)}ms`,
		);
	} catch (err) {
...
async function handleOffer(msg) {
	...
	pc.ontrack = (ev) => {
		...
		video
			.play()
			.catch((err) => console.warn(`[${testId}] video.play() rechazado`, err));
	...
	const result = await new Promise((resolve) => {
		...
		video.addEventListener("seeked", () => {
			...
			resolve({
				ok,
				reason: ok
					? `seeked to ${video.currentTime.toFixed(1)}s`
					: "seek landed off-target",
			});
		});
...
async function runAutoplayTest() {
	...
	try {
		...
		setStatus(
			"autoplay",
			ok ? "pass" : "fail",
			ok ? "played without interaction" : "play() resolved but no progress",
		);
	} catch (err) {
...
function startStabilityTest(video) {
	...
	const uiInterval = setInterval(() => {
		...
		document.getElementById("st-elapsed").textContent =
			`${(elapsedMs / 1000).toFixed(0)}s / 600s`;

		if (video.getVideoPlaybackQuality) {
			const q = video.getVideoPlaybackQuality();
			document.getElementById("st-dropped").textContent =
		...
		if (rtts.length) {
			...
			document.getElementById("st-rtt").textContent =
				`${avg.toFixed(0)}ms / ${max.toFixed(0)}ms`;
		}
